...
방어의 핵심은 취약점 자체를 제거하거나 실행을 억제하는 것이다. 신속한 패치 관리, EDR(Endpoint Detection & Response)의 행위 기반 차단, DEP/ASLR 같은 익스플로잇 완화 기술, 애플리케이션 화이트리스팅, 매크로 실행 정책 강화가 대표적이다. 제로데이(Zero-day) 취약점의 경우 시그니처 기반 방어가 무력하므로, EDR의 이상행위 탐지가 실질적 방어선이 된다.

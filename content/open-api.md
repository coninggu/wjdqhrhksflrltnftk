...
```mermaid
flowchart LR
  C[클라이언트] -->|"HTTP 요청(GET/POST)"| G[API 게이트웨이]
...
- 제로트러스트·mTLS로 API 보안 강화

---

> **한 줄 요약**: 개방형 API는 표준(REST/SOAP) 기반 공개 인터페이스로 생태계를 확장하며, *OAuth 인증·게이트웨이·Rate Limiting* 등으로 OWASP API 취약점에 대응한다.

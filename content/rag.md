...
| **임베딩 모델** | 텍스트를 벡터로 변환 |
| **벡터 DB** | 임베딩 저장·유사도 검색(FAISS, Pinecone) |
...
| 구분 | RAG | 파인튜닝 |
|---|---|---|
| **지식 주입** | 외부 검색으로 프롬프트 결합 | 가중치에 내재화 |
| **최신성** | 지식베이스만 갱신 | 재학습 필요 |
| **적합** | 최신·근거 기반 사실 응답 | 형식·톤·능력 내재화 |
...
> **한 줄 요약**: RAG는 *외부 지식 검색 결과를 LLM 프롬프트에 결합* 하여 **환각을 줄이고 최신·근거 기반 답변** 을 생성하는, 재학습 없는 지식 주입 기법으로 파인튜닝과 상호 보완된다.

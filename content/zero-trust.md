...
실무적 도입 전략의 요체는 **'보호할 대상 표면(Protect Surface)을 먼저 정의'** 하는 것이다. 킨더백이 강조했듯 방대한 공격 표면(Attack Surface) 전체를 방어하려 하기보다, 조직에서 가장 중요한 데이터·자산·애플리케이션·서비스(DAAS)를 식별하고 그 주위로 마이크로 경계를 좁혀 우선 적용한다. 이후 트랜잭션 흐름을 매핑하고, 정책을 'Who/What/When/Where/Why/How'의 카호(Kipling) 방식으로 세밀하게 기술하며, 지속 모니터링으로 정책을 반복 개선한다. 이러한 단계적 접근은 기존 레거시(온프레미스 시스템·구형 프로토콜)와의 공존, 사용자 경험 저하 우려, MFA 피로(Fatigue) 공격 같은 현실적 제약을 관리하면서 전환하기 위한 현실적 해법이기도 하다.
